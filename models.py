...
class EasyObservation(BaseModel):
    ...
    step_index: int = Field(default=0, description="Current step number in the episode")
    max_steps: int = Field(default=4, description="Maximum steps allowed in this episode")
    available_actions: list[str] = Field(default_factory=list, description="Allowed action types at this step")
    revealed_sections: list[str] = Field(default_factory=list, description="Data sections currently visible to the agent")
    penalties: list[str] = Field(default_factory=list, description="Undesirable behaviors detected so far")
...
class HardObservation(BaseModel):
    ...
    step_index: int = Field(default=0, description="Current step number in the episode")
    max_steps: int = Field(default=6, description="Maximum steps allowed in this episode")
    available_actions: list[str] = Field(default_factory=list, description="Allowed action types at this step")
    revealed_sections: list[str] = Field(default_factory=list, description="Data sections currently visible to the agent")
    penalties: list[str] = Field(default_factory=list, description="Undesirable behaviors detected so far")
    score: float = Field(default=0.0)
